...
# --- Scene Objects (Simplified for brevity, assuming existing classes like Sphere/Cube) ---
...
def create_scene():
    ...
    cube = Cube(center=[2.5, -1, -8], size=2, color=[0, 255, 0])
...
def setup_slit(position_z=-6, slit_width_x=6.0, slit_height_y=0.5):
    """
    Sets up the bounds for the horizontal slit filter.
    The slit is centered at (0, 0, position_z) and acts as a filter for scattered rays.
    """
    # Assuming a large overall plane size (e.g., 10x10) for context, 
    # but the filter only cares about the slit aperture (min/max x/y).
    
    return {
        'z': position_z,
        'min_x': -slit_width_x / 2,
        'max_x': slit_width_x / 2,
        'min_y': -slit_height_y / 2,
        'max_y': slit_height_y / 2,
        'width': slit_width_x,
        'height': slit_height_y
    }

def check_slit_passage(ray_origin, ray_direction, slit_bounds):
    """
    Checks if a scattered ray passes through the horizontal slit opening.
    """
    slit_z = slit_bounds['z']
    
    # Ensure the ray is traveling towards the hologram plane (negative Z direction)
    ...
    t = (slit_z - ray_origin[2]) / ray_direction[2]
    
    if t < EPSILON:
        # Slit plane is behind the scattered ray origin or too close
        return False 

    P = ray_origin + t * ray_direction
    x, y = P[0], P[1]
    
    # Check if (x, y) falls within the slit aperture at Z=slit_z
    x_ok = slit_bounds['min_x'] <= x <= slit_bounds['max_x']
    y_ok = slit_bounds['min_y'] <= y <= slit_bounds['max_y']
...
# --- Visualization Function (Updated to show slit) ---

def visualize_scene(objects, light_source, plane_bounds, slit_bounds, rays_to_plot):
    """
    Creates a 3D visualization of the scene, hologram plane, slit, and selected rays.
    """
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')
    ax.set_title("Diffuse Ray Tracer Visualization (Slit Filtered Rays)")

    ax.scatter(light_source['position'][0], light_source['position'][1], light_source['position'][2],
               color='yellow', s=100, label='Light Source')

    # Plot scene objects (Sphere and Cube)
    sphere = objects[0]
    ...
    # Plot the Slit (representing the aperture opening)
    slit_z = slit_bounds['z']
    x_slit = np.linspace(slit_bounds['min_x'], slit_bounds['max_x'], 2)
    y_slit = np.linspace(slit_bounds['min_y'], slit_bounds['max_y'], 2)
    X_slit, Y_slit = np.meshgrid(x_slit, y_slit)
    Z_slit = np.full(X_slit.shape, slit_z)
    ax.plot_surface(X_slit, Y_slit, Z_slit, color='magenta', alpha=0.6, label='Slit Aperture')
...
def main():
    ...
    slit_bounds = setup_slit(position_z=-6, slit_width_x=4.0, slit_height_y=0.2)
    ...
    NUM_RAYS = 500000 # Increased ray count for better sampling with the slit filter
    ...
    print("Hologram Plane Size: {}x{} pixels (Z={})".format(W, H, plane_bounds['z']))
    print("Slit Filter: X width={:.2f}m, Y height={:.2f}m (Z={})".format(slit_bounds['width'], slit_bounds['height'], slit_bounds['z']))
    print("Total Rays Sampled: {}".format(NUM_RAYS))
    print("Starting diffuse ray-tracing simulation with slit filter...")
        
    for k in range(NUM_RAYS):
        # 1. Generate primary ray direction from the light source (RESTRICTED SAMPLING)
        ...
        if hit_object is not None and closest_t != float('inf'):
            ...
            # 4. CRUCIAL FILTER STEP: Trace scattered ray (Object -> Slit)
            if not check_slit_passage(scattered_origin, scattered_direction, slit_bounds):
                continue # Ray was blocked by the plane surrounding the slit
            
            # 5. Trace filtered scattered ray (Slit -> Plane)
            result = intersect_plane(scattered_origin, scattered_direction, plane_bounds)
            
            if result is not None:
                r, c, t_plane = result
                
                # 6. Calculate and accumulate intensity and direction
        ...
        # Print progress
        if (k + 1) % 20000 == 0 or (k + 1) == NUM_RAYS:
            print("Processed {} of {} rays...".format(k + 1, NUM_RAYS))
            
    # --- 7. Final Calculation and Data Output ---
    ...
    # Data is still written to file (omitting file writing logic for brevity)
    # ... (File writing section is omitted but remains in the full code) ...
                
    # --- 8. Image Generation and Output ---
    
    # 8.1 Save Intensity Image (First grayscale PNG)
    INTENSITY_SCALE_FACTOR = 250.0 
    ...
    # 8.2 Calculate and Save Angle Map (Second grayscale PNG)
    ...
    save_grayscale_image(angle_data, hit_mask, "angle_map.png", 1.0) 
    
    # Now, visualize the scene
    print("Generating 3D visualization...")
    visualize_scene(objects, light, plane_bounds, slit_bounds, rays_to_visualize)
